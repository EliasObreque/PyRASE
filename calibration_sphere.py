...
# F_s = -P_srp * A_proj * ((absor + diffuse) * (ray_dir + 2.0 * normal_cell / 3) + 2.0*spec*cos_th[:,None] * normal_cell)
T_s = np.cross(r_m, F_s)

# === Sums ===
F_drag_total = F_d.sum(axis=0)
T_drag_total = T_d.sum(axis=0)
F_srp_total  = F_s.sum(axis=0) / 405.38
T_srp_total  = T_s.sum(axis=0)

print("Drag  F:", F_drag_total, "\n","Drag  T:", T_drag_total)
print("SRP   F:", F_srp_total,  "\n","SRP   T:", T_srp_total)
...
F_srp_ref = P_srp * A_proj_2d_sphere *ray_dir * Cr / 405.38
print(Cr, F_srp_ref)
rel_err = np.linalg.norm(F_srp_total - F_srp_ref)/np.linalg.norm(F_srp_ref)
print("Relative error (SRP vs projected-area ref):", rel_err * 100)
